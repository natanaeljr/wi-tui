...
impl Rect {
  ...
  pub fn from_size((x, y): (usize, usize), (width, height): (usize, usize)) -> Option<Self> {
    if width > 0 && height > 0 {
  ...
  pub fn from_size_unchecked((x, y): (usize, usize), (width, height): (usize, usize)) -> Self {
    Self {
      x: (x, x + width - 1),
      y: (y, y + height - 1),
    }
  ...
  pub fn with_x0(&self, x0: usize) -> Option<Self> {
  ...
  /// If unsure, use the "checked" version.
  pub fn with_x0_unchecked(&self, x0: usize) -> Self {
    Self {
      x: (x0, self.x.1),
      y: self.y,
    }
  ...
  ///
  /// Checks that the new rectangle will be valid.
  pub fn with_y0(&self, y0: usize) -> Option<Self> {
  ...
  /// If unsure, use the "checked" version.
  pub fn with_y0_unchecked(&self, y0: usize) -> Self {
    Self {
      x: self.x,
      y: (y0, self.y.1),
    }
  ...
  //   y0.to_usize().and_then(|y0| self.with_y0_unchecked(y0).checked())
  // }
  //
  // /// Offset the y.0 value.
  // ///
  ...
  //   let y0 = self.y.0 as i32 + offset as i32;
  //   self.with_y0_unchecked(y0 as usize)
  // }
  //
  ...
  //   x0.to_usize().and_then(|x0| self.with_x0_unchecked(x0).checked())
  // }
  //
  // /// Offset the x.0 value.
  // ///
  ...
  //   let x0 = self.x.0 as i32 + offset as i32;
  //   self.with_x0_unchecked(x0 as usize)
  // }
  //
